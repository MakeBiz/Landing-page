...
## IndexNow (добавлен 12 сен 2026)
- Файл-ключ лежит в корне сайта: `<ключ>.txt`, его содержимое равно имени. Это и есть подтверждение владения, отдельная верификация в Bing не нужна
- `timeweb-server.js` после старта один раз отправляет адреса из sitemap.xml на api.indexnow.org. Повтор не шлётся: отпечаток sitemap запоминается во временной папке контейнера
- Выключить можно переменной окружения `INDEXNOW=off`
- Адреса берутся из sitemap.xml, поэтому после добавления страниц достаточно обновить sitemap
